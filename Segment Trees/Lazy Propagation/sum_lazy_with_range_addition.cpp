...
// Find Sum Segment tree with Range Addition using Lazy
class seg_tree
{
public:
    int n;
    ...
    ll add(int qs, int qe, ll dat, int s = 0, int e = -1, int ci = 0)
    {
        ...
        int mid = (s + e) / 2;
        //** Danger - Change this, if change in type of updates **
        tr[ci] = add(qs, qe, dat, s, mid, 2 * ci + 1) + add(qs, qe, dat, mid + 1, e, 2 * ci + 2);
        return tr[ci];
    }
};
int main()
{
    ...
    cout << sg.getsum(2, 4) << endl;
    sg.add(0, 4, 1);
    sg.add(0, 3, 1);
    sg.add(0, 2, 1);
    sg.add(0, 1, 1);
    sg.add(0, 0, 1);
    cout << sg.getsum(0, 4) << endl;
    cout << sg.getsum(1, 1) << endl;
    cout << sg.getsum(2, 2) << endl;
    cout << sg.getsum(3, 3) << endl;
    cout << sg.getsum(4, 4) << endl;
    return 0;
}