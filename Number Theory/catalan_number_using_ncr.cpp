...
vector<ll> preprocessing(int mxn) // O(mxn)
{
   vector<ll> pre(mxn + 1);
   for (int i = 0; i <= mxn; i++)
   {
...
map<ll, ll> primefactorization(int nn, vll &spf) // O(log{nn})
{
   map<ll, ll> myp;
   while (nn > 1)
   {
...
ll catalan(ll r, ll mdd) // O(nlogn)
{
   ll n = 2 * r;
   ...
   map<ll, ll> prime = primefactorization(r + 1, spf);
   for (auto i : prime)
      ans[i.first] -= i.second;
   ll ret = 1;
   for (int i = 0; i <= n; i++)
